...
public class AsymmetricCryptoDemo {
	...
	public static void main(String[] args) {
		...
		try {
			...
			AsymmetricKeyGen keygen = new AsymmetricKeyGen("RSA", 2048);
			...
			while (choice != -1) {
				...
				case 0:
					Arrays.asList(filelist).forEach(file -> {
						try {
							CryptoFunctions.encryptFile(file, keygen.getCipher(), keygen.getPublicKey());
						} catch (Exception e) {
				...
				case 1:
					...
					if (encryptValue == JFileChooser.APPROVE_OPTION) {
						...
						try {
							CryptoFunctions.encryptFile(selectedFile, keygen.getCipher(), keygen.getPublicKey());
						} catch (Exception e) {
				...
				case 2:
					Arrays.asList(filelist).forEach(file -> {
						try {
							CryptoFunctions.decryptFile(file, keygen.getCipher(), keygen.getPrivateKey());
						} catch (Exception e) {
				...
				case 3:
					...
					if (decryptValue == JFileChooser.APPROVE_OPTION) {
						...
						System.out.println(selectedFile.getAbsolutePath());
						try {
							CryptoFunctions.decryptFile(selectedFile, keygen.getCipher(), keygen.getPrivateKey());
						} catch (Exception e) {
